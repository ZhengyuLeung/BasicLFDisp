...
class get_model(nn.Module):
    ...
    def forward(self, x, info):
        x = LF_rgb2ycbcr(x)[:, 0:1, :, :, :, :]

        lf = x
        x = rearrange(x, 'b c a1 a2 h w -> b c (a1 a2) h w', a1=self.angRes, a2=self.angRes)
        b, c, _, h, w = x.shape
...
def warp(img, disp, du, dv, x_base, y_base):
    ...
    img_warped = F.grid_sample(img, 2 * flow_field - 1, mode='bilinear', padding_mode='zeros', align_corners=True)
    return img_warped
